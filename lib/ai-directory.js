...
const CATEGORY_META = {
  '1. Core Frameworks & Libraries': { icon: '🧱', color: '#0d9488', shortName: 'Core Frameworks' },
  '2. Model Codebases & Model Families': { icon: '🤖', color: '#14b8a6', shortName: 'Model Families' },
...
export function getCapabilityCounts(role) {
  const projects = loadProjects();
  const accessibleCategories = CATEGORY_ACCESS[role] || CATEGORY_ACCESS.viewer;
  const accessible = projects.filter(p => accessibleCategories.includes(p.category));
...
const INDUSTRY_META = {
  'Technology & Software': { icon: '💻', color: '#0d9488', shortName: 'Technology' },
